...
const totalValue = computed(() => {
    ...
    return gameStore.resourceList.reduce((total, resource) => {
        return total + (resource.amount * getResourceValue(resource.type))
    }, 0)
})
...
const handleBuildingSelect = (buildingKey: string) => {
    ...
    // NE PAS déduire les ressources ici - juste sélectionner le bâtiment
...
const getResourceValue = (resourceType: string): number => {
    // Basic resource values - you can make this more sophisticated
